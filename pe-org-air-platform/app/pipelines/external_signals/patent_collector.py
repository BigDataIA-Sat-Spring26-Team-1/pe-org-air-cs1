            ...
            try:
                async with session.request(method, url, json=payload, headers=headers) as resp:
                    ...
                    if resp.status == 429:
                        ...
                        logger.warning(f"429 Too Many Requests. Retry-After={retry_after}. Sleeping {wait_s}s...")
                        await asyncio.sleep(wait_s)
                        continue

                    if resp.status in (500, 502, 503, 504):
                        wait_s = 2 ** attempt
                        logger.warning(f"{resp.status} server error. Sleeping {wait_s}s then retry...")
                        await asyncio.sleep(wait_s)
                        continue
                ...
                wait_s = 2 ** attempt
                logger.warning(f"Request error: {e}. Sleeping {wait_s}s then retry...")
                await asyncio.sleep(wait_s)
...
class PatentCollector:
    """
    Analyzes the corporate patent portfolio to find AI-related innovation activity using PatentsView API.
    """
    ...
    # CPC/IPC prefixes that align with your keyword themes
    AI_CPC_PREFIXES = [
    ...
    AI_IPC_PREFIXES = [
        ...
        "G06F40", "G06F16", "G06T", "G06V", "G10L"
    ]

    def __init__(self):
        self.output_file = "patent_signals.csv"
        api_key = settings.PATENTSVIEW_API_KEY.get_secret_value() if settings.PATENTSVIEW_API_KEY else ""
        if not api_key:
            logger.warning("Missing PATENTSVIEW_API_KEY. PatentCollector will likely fail.")
    ...
    async def fetch_patents(self, company_name: str, years: int = 5) -> List[Patent]:
        ...
        logger.info(f"PatentsView: fetching patents for assignee variants={variants}, cutoff={cutoff}")
        ...
        try:
            async for patent_rows, total_hits in self.client.iter_patents_by_assignee(
                ...
                logger.info(f"Fetched page: {len(patent_rows)} records (total_hits={total_hits})")
    ...
    async def collect(self, company_name: str, years: int = 5) -> CollectorResult:
        """
        Orchestrator entry point.
        """
        logger.info(f"Scanning innovation portfolio for {company_name} using PatentsView")
        ...
        evidence_items = [
            SignalEvidenceItem(
                title=f"Patent {p.patent_number}: {p.title}",
                description=p.abstract[:2000] if p.abstract else None,
                url=f"https://patents.google.com/patent/US{p.patent_number}/en",
                tags=p.ai_categories + ["Patent", "Innovation"],
                date=p.filing_date.strftime("%Y-%m-%d") if p.filing_date else str(now.date()),
                metadata={"patent_number": p.patent_number, "cpc": p.cpc}
            )
            for p in ai_patents
        ]
        ...
        return CollectorResult(
            category=SignalCategory.INNOVATION_ACTIVITY,
            normalized_score=min(round(float(score), 2), 100),
            confidence=0.9 if len(patents) > 0 else 0.5,
            raw_value=f"{len(ai_patents)} AI patents found in last {years} years",
            source="PatentsView",
